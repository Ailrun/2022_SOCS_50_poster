\documentclass[25pt,a0paper,portrait,innermargin=0.4in,colspace=0.4in,blockverticalspace=0.4in]{tikzposter}
...
\newcommand\moeb[1]{\lstinline[flexiblecolumns=false]{#1}}
...
nth : Int -> \Box (List Int -> Int)
o<<nth>>o n =
  if n <= 0 then
    o<<box>>o(fun xs => head Int xs)
  else
    let box(C) = nth (n - 1) in
...
nth : (a' : Type) -> Int -> \Box (List 'a -> 'a)
o<<nth>>o a' n =
  if n <= 0 then
    o<<box>>o(fun xs => head a' xs)
  else
...
\begin{columns}
  ...
  \block{Metaprogramming}{
    ...
    \begin{tikzfigure*}
      \begin{tikzpicture}
        \draw (0,2) node {\moeb{box(2 + 3)}} (20,2) node {A code fragment for code \moeb{2 + 3}};
        \draw (0,0) node {\moeb{let box(C) = box(2 + 3) in C}} (20,0) node {Execution of \moeb{C} to get \moeb{5}};
      \end{tikzpicture}
    \end{tikzfigure*}
