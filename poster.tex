...
\settitle{\vbox{
  \hspace*{2in}
  \colorbox{white}{\hbox to 4in {\vbox to 4.2in {\vspace*{1.7in} \raisebox{-0.5\height}{\@titlegraphic}}}}
  \hspace*{-6.8in}
  \vbox{
...
\defineblockstyle{MyBlock}{
  ...
  bodyverticalshift=0.5in, roundedcorners=2, linewidth=5pt,
  titleinnersep=0.3in, bodyinnersep=0.4in
}{
...
ws<<openIntNth>>ws n = if n <= 0 then box(nb<<xs>>nb . nb<<head Int xs>>nb)
                         else let box(nb<<xs>>nb . C) = ws<<openIntNth>>ws (n - 1) in
...
ws<<polyNth>>ws 'a n = if n <= 0 then box(nb<<xs>>nb .^1 nb<<head 'a xs>>nb)
                         else let box(xs .^1 C) = ws<<polyNth>>ws 'a (n - 1) in
...
\begin{columns}
  ...
  \block[bodyoffsety=0.8in]{Metaprogramming with Box/Let-box}{
    \begin{tikzfigure*}
      \begin{tikzpicture}
  ...
  \block[bodyoffsety=1in]{Contextual Modality: Generalization of Necessity}{
    \begin{tikzfigure*}
      \begin{tikzpicture}
  ...
  \block[bodyoffsety=0.8in]{Levels: Necessity beyond Necessity}{
    \begin{tikzfigure*}
      \begin{tikzpicture}
        \draw (0,2.5) node {\moeb{fg<<(Gamma |-^1 A)>>fg}} (20,2.5) node[text width=10.5in,align=center] {Level 1 template depends on \(\Gamma\) of normal types};
        ...
        \draw (0,-3.5) node {\moeb{fg<<[Gamma |-^n A]>>fg}} (20,-3.5) node[text width=10.5in,align=center] {\moeb{fg<<Gamma>>fg} may contain normal types and\\ \color{red!30!white}{level \makebox[1.1in]{\(\!\!\le{}\!n\!-\!1\)} templates}};
      \end{tikzpicture}
    \end{tikzfigure*}
  ...
  \block{Acknowledgement}{\normalsize This work can be done thanks to the guidance from my supervisor, Prof. Brigitte Pientka, and the help from two other coauthors, Prof. Stefan Monnier at UdeM and MSc. Samuel G\'elineau}

  \block{References}{
    % \renewcommand{\bibname}{}
    \small
    \renewcommand{\refname}{\vskip -0.65in}
    \nocite{Jang:POPL22,Nanevski:TOCL08,Davies:ACM01}
